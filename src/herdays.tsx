...
function cleanOldLogs(logs: Record<string, EntryData>): Record<string, EntryData> {
...
// ── Blank entry — all menstrual fields null ───────────────────────────────────
const BLANK_ENTRY: EntryData = {
  flow_level: null,
  pain_level: null,
  medication: false,
  blood_color: null,
...
const FLOW_FILL_PCT: Record<number, number> = { 1: 25, 2: 50, 3: 75, 4: 100 };
...
function isEntryMeaningful(e: EntryData): boolean {
  return (
    e.flow_level !== null ||
    e.pain_level !== null ||
    e.medication === true ||
...
function getPredictedDates(settings: Settings): Set<string> {
...
function getWeeksForRange(): Date[][] {
...
// ── Sub-components ────────────────────────────────────────────────────────────
function Drop({ size = 13, color = "#a855f7" }: { size?: number; color?: string }) {
  return (
...
function NoteIcon({ color = "#7c3aed" }: { color?: string }) {
  return (
...
export default function App() {
  const [logs,         setLogs]         = useState<Record<string, EntryData>>(() => cleanOldLogs(load(SK.logs, {})));
  const [settings,     setSettings]     = useState<Settings>(() => load(SK.settings, DEFAULT_SETTINGS));
  const [pinEnabled,   setPinEnabled]   = useState<boolean>(() => load(SK.pinEnabled, false));
  const [pin,          setPin]          = useState<string>(() => load(SK.pin, ""));
  const [pinUnlocked,  setPinUnlocked]  = useState(false);
  const [showPinOverlay, setShowPinOverlay] = useState(false);
  const [pinMode,      setPinMode]      = useState<"check" | "setup">("check");
  const [pinInput,     setPinInput]     = useState("");
  const [pinError,     setPinError]     = useState("");

  const [selectedDay,    setSelectedDay]    = useState<string | null>(null);
  const [showPanel,      setShowPanel]      = useState(false);
  const [panelEntry,     setPanelEntry]     = useState<EntryData>({ ...BLANK_ENTRY });
  ...
  useEffect(() => {
    ...
    const last = load<number>(SK.lastPinCheck, 0);
  ...
  // Toggle: clicking an active value deselects it back to null
  const toggleField = (field: keyof EntryData, val: number | string) => {
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg,#e8d5f5 0%,#f0e6ff 30%,#dce8ff 60%,#f5e0ed 100%)", fontFamily: "'Inter',sans-serif" }}>
      ...
      <div style={{ position:"sticky", top:0, zIndex:50, ...glass, borderRadius:"0 0 20px 20px", padding:"13px 18px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        ...
        <div style={{ display:"flex", gap:8 }}>
          ...
          <Btn onClick={() => { setPinMode(pinEnabled ? "check" : "setup"); setPinInput(""); setPinError(""); setShowPinOverlay(true); }}>
            {pinEnabled ? "🔒" : "🔓"}
          </Btn>
        </div>
      </div>
            ...
            <div key={wi} ref={weekContainsToday ? todayRowRef : null}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:3, marginBottom:3 }}>
                {week.map((day, di) => {
                  ...
                  const hasNoteOnly = log && !hasFlow && (log.note ?? "").trim() !== "" && pinUnlocked;
                  ...
                  return (
                    <div
                      key={di}
                      onClick={() => { if (isClickable) openDay(ds); }}
                      style={{ height:48, borderRadius:12, border, overflow:"hidden", cursor:isClickable?"pointer":"default", position:"relative", opacity:inMonth?1:0.28, background:isToday?"rgba(167,139,250,0.15)":"rgba(255,255,255,0.28)" }}
                    >
                      {fillPct > 0 && (
      ...
      {showPanel && (
        <div style={overlayStyle} onClick={e => { if (e.target === e.currentTarget) setShowPanel(false); }}>
          <div style={{ ...glass, width:"100%", maxWidth:480, borderRadius:"24px 24px 0 0", padding:22, paddingBottom:32, animation:"slideUp 0.3s cubic-bezier(0.34,1.56,0.64,1)", maxHeight:"90vh", overflowY:"auto" }}>
            ...
            {/* Blood Color — only when flow is selected */}
            {panelEntry.flow_level !== null && (
              <div style={{ marginBottom:14 }}>
            ...
            <div style={{ marginBottom:14 }}>
              ...
              <div style={{ display:"flex", gap:5 }}>
                {[0,1,2,3,4,5].map(v => {
                  ...
                  const isNone = v === 0;
                  return (
                    <button key={v} onClick={() => toggleField("pain_level", v)} style={{ flex:1, height:46, borderRadius:12, border:active?"2px solid #db2777":"1.5px solid rgba(219,39,119,0.25)", background:isNone?(active?"rgba(52,211,153,0.22)":"rgba(255,255,255,0.55)"):(active?"rgba(219,39,119,0.15)":"rgba(255,255,255,0.55)"), color:active?(isNone?"#065f46":"#831843"):(isNone?"#059669":"#9d174d"), fontWeight:700, cursor:"pointer", fontSize:11, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:1 }}>
        ...
        <div style={overlayStyle} onClick={e => { if (e.target === e.currentTarget) setShowBackup(false); }}>
          <div style={{ ...glass, width:"100%", maxWidth:480, borderRadius:"24px 24px 0 0", padding:22, paddingBottom:32, animation:"slideUp 0.3s ease" }}>
            ...
            <div style={{ marginBottom:14 }}>
              ...
              {exportStr && <textarea readOnly value={exportStr} rows={4} onClick={e => (e.target as HTMLTextAreaElement).select()} style={{ width:"100%", marginTop:8, borderRadius:12, border:"1.5px solid rgba(124,58,237,0.2)", background:"rgba(255,255,255,0.6)", padding:"10px 12px", fontSize:11, color:"#3b0764", resize:"none", boxSizing:"border-box", fontFamily:"monospace" }} />}
            </div>
            <div style={{ marginBottom:16 }}>
      ...
      {showPinOverlay && (
        <div style={{ ...overlayStyle, alignItems:"center" }}>
          <div style={{ ...glass, width:320, borderRadius:24, padding:32, textAlign:"center" }}>
            ...
            {pinMode === "check" && (
              <button onClick={() => { save(SK.pinEnabled, false); setPinEnabled(false); setPinUnlocked(true); setShowPinOverlay(false); }} style={{ marginTop:12, background:"none", border:"none", color:"#6b21a8", fontSize:12, fontWeight:600, cursor:"pointer", textDecoration:"underline" }}>
